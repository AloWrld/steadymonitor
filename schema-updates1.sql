...
-- Add missing columns from Excel logic
...
ADD COLUMN IF NOT EXISTS previous_class VARCHAR(50), -- For tracking promotions
ADD COLUMN IF NOT EXISTS promotion_date TIMESTAMP WITH TIME ZONE; -- When promoted

-- Remove any reference to Program C (pocket money is feature flag, not program)
-- Update existing program_membership: if contains 'C', remove it
UPDATE customers 
...
-- Ensure program_membership is single program (A or B or none)
UPDATE customers 
SET program_membership = 
  CASE 
...
-- Add constraint for single program
ALTER TABLE customers 
...
-- For tracking special sales (pocket money, notes, etc.)
ALTER TABLE sales 
...
-- Add missing columns for frequency logic
ALTER TABLE allocations 
ADD COLUMN IF NOT EXISTS next_due_date TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS specific_days VARCHAR(100), -- e.g., "Monday,Wednesday,Friday"
ADD COLUMN IF NOT EXISTS given_date TIMESTAMP WITH TIME ZONE; -- renamed from last_given_date

-- Update frequency constraint to match Excel options
ALTER TABLE allocations 
...
-- Ensure only A and B programs exist
...
-- 5. CREATE PROMOTION HISTORY TABLE (NEW)
-- Track class changes and promotions
...
-- Add batch tracking for class-wide operations
ALTER TABLE batches 
...
ADD COLUMN IF NOT EXISTS new_class VARCHAR(50); -- For promotion batches
...
-- 8. FUNCTION FOR PROMOTING LEARNERS
...
BEGIN
    ...
    -- Process each learner in the class
    FOR v_learners IN 
...
BEGIN
    IF OLD.class IS DISTINCT FROM NEW.class THEN
        INSERT INTO promotion_history (
            ...
            'system', -- Will be overridden by actual user in application
